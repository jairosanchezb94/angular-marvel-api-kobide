...
export class CharactersService {
  ...
  getAllCharacters(offset: number, limit: number) {
    ...
    let subject = result.pipe(tap(() => { this.haveRequest = false; this._loadingService.hide() }))
      ...
      //.subscribe((res: any) => this.characters.next(res.data.results.filter((item: any) => item.thumbnail.path != 'http://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available' && item.description != '')))
    return subject;
  }
